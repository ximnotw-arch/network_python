...
async def fetch_all_async(urls: list[str]) -> list[str]:
    ...
    """
    coroutines = [fetch_one_async(url) for url in urls]
...
async def run_task_group(names: list[str]) -> dict[str, str | None]:
    ...
    try:
        async with asyncio.TaskGroup() as tg:
            for name in names:
                tasks_dict[name] = tg.create_task(
    ...
    for name, task in tasks_dict.items():
        if task.exception() is not None:
            ...
            final_results[name] = task.result()
            all_failed = False
            
    if all_failed and names: 
        return {}
...
async def delayed_fetch(url: str, delay: float):
        await asyncio.sleep(delay)
...
async def fetch_with_timeout(url: str, delay: float, timeout: float) -> str:
    ...
    """
    try:
        result = await asyncio.wait_for(delayed_fetch(url, delay), timeout=timeout)
        return result
...
async def fetch_as_completed(tasks: list[tuple[str, float]]) -> list[str]:
    ...
    """
    tasks_list = [fast_or_slow(name, delay) for name, delay in tasks]
    results = []
...
async def async_process_numbers(numbers: list[int], max_workers: int = 4) -> list[int]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        tasks = [loop.run_in_executor(executor, blocking_compute, num) for num in numbers]
